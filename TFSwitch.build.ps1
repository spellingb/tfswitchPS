...
task GetNextVersion {
    ...
    try {
        $env:CurrentOnlineVersion = [Version](Find-Module -Name $env:BHProjectName -Repository PSGallery -ErrorAction Stop).Version
        ...
        $env:CurrentOnlineVersion = 'unavailable'
    }
}
#endregion Setup
...
task PrepareTests Init, {
    ...
    Copy-Item -Path "$env:BHProjectPath/PSScriptAnalyzerSettings.psd1" -Destination $env:BHBuildOutput -Force -ErrorAction SilentlyContinue
}
